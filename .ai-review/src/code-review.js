...
const DEFAULT_ARCHITECTURE_FILE = './github/ai-code-review.md';
...
async function main() {
  ...
  try {
    ...
    // Resolve o arquivo de regras arquiteturais (env var ou caminho default do projeto)
    const defaultArchFile = path.join(repoRoot, DEFAULT_ARCHITECTURE_FILE);
    const architectureFilePath = process.env.ARCHITECTURE_FILE ||
      (fs.existsSync(defaultArchFile) ? defaultArchFile : undefined);
    console.log(`Documento de arquitetura encontrado: ${architectureFilePath}`)
    review = await reviewDiff(combinedDiff, { architectureFilePath });
  } catch (err) {
